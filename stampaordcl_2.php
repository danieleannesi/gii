...
if(isset($_SERVER['argv'][1])) {
   $idt=$_SERVER['argv'][1];  
   $numid=$_SERVER['argv'][2];  
   }
//
//legge documento
  $qr="SELECT * FROM documtes LEFT JOIN clienti ON DOCT_CLIENTE=CodCli WHERE DOCT_ID=$idt";
  ...
  $agente=$row["agente"];
  $sconto=$row["sconto"];
  $iva=$row["DOCT_TRAT_IVA"];
  $totalerae=$row["DOCT_IMPORTO_RAE"];
  $flag_foto=$row["DOCT_FOTO"];
  $flag_scheda=$row["DOCT_SCHEDA"];
...
$localita=$row["citta"];
$prov=$row["prov"];
if(strlen(trim($ragsoc))==0)
  {
  $ragsoc=$row["DOCT_CRAG"];
  $indirizzo=$row["DOCT_CIND"];
  $localita=$row["DOCT_CLOC"];
  $cap=$row["DOCT_CCAP"];
  $prov=$row["DOCT_CPR"];
  }  
$partiva=$row["PIVA"];
$spese=$row["DOCT_TRASPORTO"];
...
$totpre=$row["DOCT_TOT_FATTURA"];
$des_ragsoc=$row["DOCT_DES_RAG_SOC"];
$des_indi=$row["DOCT_DES_INDIRIZZO"];
$des_loc=$row["DOCT_DES_LOC"];
$des_cap=$row["DOCT_DES_CAP"];
$des_pr=$row["DOCT_DES_PR"];
$spedizione=$row["DOCT_VETTORE"];
$trasporto=$row["DOCT_IND_VETTORE"];
$banca=$row["DOCT_BANCA"];
$porto=$row["DOCT_CAUSALE"];
$imballo=$row["DOCT_ASPETTO"];
$note=stripcslashes($row["DOCT_NOTE"]);
$caparra=$row["DOCT_ACC_SCALATO"];
$acconto=$row["DOCT_ACCONTO"];

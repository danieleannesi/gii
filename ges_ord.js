   ...
   for (var i=0;i<document.documenti.elements.length;i++)
    ...
    request=$.ajax({
	  ...
	  success: function(res) {
	  	 ...
	  	 if(res.numerofo>0)
	  	   {
		   	fofo="Emesso Ordine Fornitore " + res.numerofo;
		   }
	     $("#dialog").html('<p>Dati Salvati Correttamente<br>' + fofo + "</p>").dialog({modal: true, buttons: '',title: "Salva Dati" });
         $("#dialog").dialog("open");
	     },
...
if(idt>0) {
	var h="<iframe src=\"stampaord.php?idt=" + idt + "\" width=\"800px\" height=\"700px\"></iframe>";
    ...
    request=$.ajax({
	  ...
	  success: function(result) {
	  	  if(result.errore=='0')
	  	    ...
	  	    $('#cliente').val(result.ORDI_CLIENTE); 
	  	    $('#ragsoc').val(result.cf_ragsoc); 
	  	    $('#ute').val(result.ORDI_UTENTE);
		    $('#age').val(result.ORDI_AGENTE);
